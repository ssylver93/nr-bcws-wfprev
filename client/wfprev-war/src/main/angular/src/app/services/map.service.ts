...
export class MapService {
  private map!: L.Map;
  identifyCallback: any;
  identifyDoneCallback: any;
  ...
  public patch(): Promise<any> {
    try {
      const mapService = this;
      const include = (window as any)['include'];
      const SMK = (window as any)['SMK'];
      ...
      if (!this.patchPromise) {
        this.patchPromise = Promise.resolve()
          ...
          .then(() => {
            ...
            SMK.TYPE.Layer['wms'].prototype.canMergeWith = function(other: { config: { combiningClass: any; }; }) {
              return (
            ...
            SMK.TYPE.Layer['wms']['leaflet'].prototype.canMergeWith = function(
              other: { config: { combiningClass: any; }; },
            ) {
              return (
                this.config.combiningClass &&
                this.config.combiningClass === other.config.combiningClass
            ...
            SMK.TYPE.Layer['wms']['leaflet'].prototype.getFeaturesInArea =
              function(area: { geometry: { coordinates: any[][]; }; }, view: any, option: any) {
                ...
                return mapService
                  ...
                  .then((response: any) => {
                    ...
                    return response.features.map((f: { title: string; properties: { [x: string]: any; }; }, i: number) => {
